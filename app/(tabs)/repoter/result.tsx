  ...
  const handleSaveReport = async () => {
    try {
      ...
      const payload = {
        ...
        updatedAt: serverTimestamp(),
        content: content || '',
        emotionSummary: safeselectedEmotion,
        summary: content || '',
        completed: true,
      };
  
      // 기존 addDoc 대신 setDoc 사용 (같은 달이면 덮어쓰기)
      await setDoc(doc(db, 'users', user.uid, 'reports', key), payload);
  
      Alert.alert('저장 완료', `${key} 회고 이미지가 저장되었습니다!`);
      router.replace('/(tabs)/repoter/detail'); // 결과 화면으로 이동
    } catch (e) {
      console.error('report save error:', e);
      Alert.alert('오류', '리포트 저장 중 문제가 발생했어요.');
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.buttonRow1}>
        ...
        <View style={styles.centerRow}>
            <Text style={styles.headerTitle}>이 달의 회고 메모</Text>
        <View style={{ flex: 1 }} />
...
const styles = StyleSheet.create({
  ...
  textalign: { width: '100%', alignItems: 'center', paddingHorizontal: 30 },
